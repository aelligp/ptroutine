...
if Nx <= 10 && Nz <= 10  % create 0D plots
    ...
    fh1 = figure(1); 
    subplot(5,1,1)
    plot(hist.time/hr,hist.T(:,2),'LineStyle',LS{i},'LineColor','k','LineWidth',2); axis xy tight; box on;
    title('$T [^\circ$C]',TX{:},FS{:}); set(gca,TL{:},TS{:});
    subplot(5,1,2)
    plot(hist.time/hr,hist.cx(:,2)*100,'LineStyle',LS{i},'LineColor','b',hist.time/hr,hist.cm(:,2)*100, 'r',hist.time/hr,hist.c(:,2)./(1-hist.f(:,2))*100, 'k','LineWidth',2); axis xy tight; box on;
    title('$\bar{c}/(1-f)$ [wt\% SiO$_2$]',TX{:},FS{:}); set(gca,TL{:},TS{:});
    subplot(5,1,3)
    plot(hist.time/hr,hist.vm(:,2)*100,'LineStyle',LS{i},'LineColor', 'r',hist.time/hr,hist.v(:,2)./(1-hist.x(:,2))*100, 'k','LineWidth',2); axis xy tight; box on;
    title('$\bar{v}/(1-x)$ [wt\% H$_2$O]',TX{:},FS{:}); set(gca,TL{:},TS{:});
    subplot(5,1,4)
    plot(hist.time/hr,hist.chi(:,2)*100.*(hist.chi(:,2)>1e-9),'LineStyle',LS{i},'LineColor', 'k','LineWidth',2); axis xy tight; box on;
    title(['$\chi$ [vol\%]'],TX{:},FS{:}); set(gca,TL{:},TS{:});
    subplot(5,1,5)
    plot(hist.time/hr,hist.phi(:,2)*100.*(hist.phi(:,2)>1e-9),'LineStyle',LS{i},'LineColor', 'k','LineWidth',2); axis xy tight; box on;
